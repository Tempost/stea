...
export const shows = router({
  ...
  get: procedure
    ...
    .query(async ({ input, ctx }) => {
      try {
        return findFirst('Show', { where: { ...input } }, ctx.prisma);
      } catch (error) {
  ...
  add: dashboardProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      return create('Show', { data: { ...input } }, ctx.prisma);
    }),

  update: dashboardProcedure
    .input(ShowUpdateArgsSchema)
    .mutation(async ({ input, ctx }) => {
      return update('Show', input, ctx.prisma);
    }),

  remove: dashboardProcedure
    .input(z.array(z.string().cuid()))
    .mutation(async ({ input, ctx }) => {
      return removeMany('Show', { where: { uid: { in: input } } }, ctx.prisma);
    }),
});
