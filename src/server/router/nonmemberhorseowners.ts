...
export const nonMemberHorseOwners = router({
  ...
  get: procedure.input(z.object({ ownerName: z.string() })).query(async ({ input, ctx }) => {
  ...
  add: procedure.input(AddOwnerInput).mutation(async ({ input: { owner, horses }, ctx }) => {
  ...
  remove: dashboardProcedure.input(z.object({ ownerName: z.string() })).mutation(async ({ input, ctx }) => {
...
async function upsertOwner({ owner, horses }: AddOwnerInput, db: MyPrismaClient) {
  try {
